local _, GraalHelper = ...

if GetLocale() ~= "deDE" then return end -- "deDE": German (Germany)

GraalHelper.L = {
    actions = "Aktionen",
    activeFunctionality = "Aktivieren",
    addonLoaded = "Wenn Sie /gh eingeben, wird das Menü geöffnet.",
    alerts = "Warnungen",
    bothLocked = "Warnungen gesperrt.",
    bothUnlocked = "Warnungen entsperrt.",
    chooseChat = "Chat-Auswahl",
    chooseMinDuration = "Mindestzauberdauer zum Unterbrechen (ignoriert kürzere)",
    chooseSound = "Klangwahl",
    disarmFound = "Entwaffnung aktiv",
    disarmLine = "Kampf ohne Waffe - Entwaffnung aktiv!",
    disarmSection = "Entwaffnung",
    disarmTitle = "Entwaffnung!",
    dispelFound = "Bannung möglich",
    dispelLine = "Entferne einen Effekt von deinem Ziel - Bannung möglich!",
    dispelSection = "(BETA) Bannung",
    dispelTitle = "(BETA) Bannung!",
    dispelTypeMagic = "Magie",
    dispelTypePoison = "Gift",
    dispelTypeDisease = "Krankheit",
    dispelTypeCurse = "Fluch",
    dispelTypeEnrage = "Wut",
    displayDuration = "Anzeigebehdauer",
    enableSound = "Aktiviert den Ton",
    errors = {
        options = {
            null = "Die Optionen sind nicht geladen."
        },
    },
    fearFound = "Furcht aktiv",
    fearLine = "Furcht aktiv!",
    fearSection = "Furcht",
    fearTitle = "Furcht!",
    globalCameraMaxZoom = "Maximale Kameradistanz",
    globalSection = "Globale Einstellungen",
    guild = "Gilde",
    hunterPackAspectFound = "Rudelaspekt aktiv",
    hunterPackAspectLine = "Rudelaspekt aktiv, entferne ihn vor dem Kampf!",
    hunterPackAspectSection = "Rudelaspekt",
    hunterPackAspectTitle = "Rudelaspekt!",
    kickFound = "Der gewirkte Zauber kann unterbrochen werden",
    kickLine = "Zauberunterbrechung möglich!",
    kickSection = "Zauberunterbrechung",
    kickTitle = "Zauberunterbrechung",
    lockWindow = "Warnung gesperrt",
    menuTitle = "GraalHelper",
    missNotifierSection = "Benachrichtigung bei Fehlschlägen",
    noSpellsTracked = "Bisher wurde noch kein Zauber verfolgt.",
    notifiers = "Benachrichtigungen",
    party = "Gruppe",
    raid = "Schlachtzug",
    raidOrPartyOrNothing = "Schlachtzug/Gruppe",
    reflectFound = "Aktiver Reflexionszauber",
    reflectLine = "Wirke keine Zauber - Reflexion aktiv!",
    reflectSection = "Warnung : Reflexionszauber",
    reflectTitle = "Reflexionszauber!",
    rootFound = "Bewegungsunfähigkeit aktiv",
    rootLine = "Bewegungsunfähigkeit aktiv!",
    rootSection = "Bewegungsunfähigkeit",
    rootTitle = "Bewegungsunfähigkeit!",
    say = "Sagen",
    sayCheck = "Im Chat anzeigen",
    scale = "Warnungsgröße",
    settings = "Einstellungen",
    showBuffNames = "Zeigt die Namen der Buffs",
    silenceFound = "Stille aktiv",
    silenceLine = "Zauber können nicht gewirkt werden - Stille aktiv!",
    silenceSection = "Stille",
    silenceTitle = "Stille!",
    slashHide = "/gh hide - Blendet Warnungen aus",
    slashLock = "/gh lock - Gesperrte Warnungen",
    slashMenu = "/gh - Öffnet das Menü",
    slashTest = "/gh test - Test: Zauberdiebstahl + Reflexion",
    slashUnlock = "/gh unlock - Warnungen entsperrt",
    spellsHint = "Hier erscheinen nur verfolgte Zauber, die das Add-on sieht.",
    spellsSection = "Zauber verfolgt",
    spellstealFound = "Stiehlbare Buffs gefunden",
    spellstealLine = "Buffs sind bereit, gestohlen zu werden!",
    stealSection = "Zauberdiebstahl",
    stealTitle = "Zauberdiebstahl!",
    stunFound = "Betäubung aktiv",
    stunLine = "Betäubung aktiv!",
    stunSection = "Betäubung",
    stunTitle = "Betäubung!",
    summonNotifierSection = "Beschwörungsbenachrichtigung",
    test = "Test",
    trackDisarm = "Entwaffnung",
    trackFear = "Furcht",
    trackKick = "Unterbrechen",
    trackReflect = "Reflexion",
    trackRoot = "Bewegungsunfähigkeit",
    trackSilence = "Stille",
    trackSteal = "Stehlen",
    trackStun = "Betäubung",
    trackUnknow = "Unbekannt",
    welcomeActive = "Willkommensnachricht aktivieren",
    welcomeLock = "Willkommensfenster sperren",
    welcomeNotShow = "Nicht mehr anzeigen",
    welcomeScale = "Skalierung der Willkommensmeldung",
    welcomeSentences = {
        "Wo ist das Hühnchen?",
        "Das ist nicht falsch.",
        "Fett ist das Leben.",
        "Wir haben die Schnauze voll!"
    },
    welcomeTestButton = "Willkommen testen",
    welcomeTitle = "Willkommen",
    yell = "Schreien",

    MATCH = {
        SPELL_REFLECTION_NAMES = {
            ["Dunkle Hülle"] = true,
            ["Zauberreflexion"] = true,
            ["Reflektierender Magieschild"] = true,
        },
        SPELL_SILENCE_NAMES = {
            ["Stille"] = true,
            ["Zaubersperre"] = true,
            ["Arkaner Strom"] = true,
            ["Gegenzauber - zum Schweigen gebracht"] = true,
            ["Tritt - zum Schweigen gebracht"] = true,
            ["Ohrenbetäubendes Gebrüll"] = true
        },
    },
}
