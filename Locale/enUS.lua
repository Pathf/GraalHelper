local _, GraalHelper = ...
-- "enGB": English (Great Britain) if returned, can substitute 'enUS' for consistancy
-- "enUS": English (America)

GraalHelper.L = {
    actions = "Actions",
    activeFunctionality = "Activate",
    addonLoaded = "loaded. /gh opens the menu",
    alerts = "Alerts",
    bothLocked = "Both windows locked.",
    bothUnlocked = "Both windows unlocked.",
    chooseChat = "Chat selection",
    chooseMinDuration = "Min. spell duration to interrupt (ignores shorter ones)",
    chooseSound = "Choose sound",
    disarmFound = "Disarm effect active",
    disarmLine = "Fighting bare-handed - Disarm active!",
    disarmSection = "Disarm effect",
    disarmTitle = "Disarm effect!",
    dispelFound = "Dispel possible",
    dispelLine = "Remove a status from your target - Dispel possible!",
    dispelSection = "(BETA) Dispel",
    dispelTitle = "(BETA) Dispel!",
    dispelTypeMagic = "Magic",
    dispelTypePoison = "Poison",
    dispelTypeDisease = "Disease",
    dispelTypeCurse = "Curse",
    dispelTypeEnrage = "Enrage",
    displayDuration = "Display duration",
    enableSound = "Enable sound",
    errors = {
        options = {
            null = "The options are not loaded."
        },
    },
    fearFound = "Fear effect active",
    fearLine = "Fear active!",
    fearSection = "Fear effect",
    fearTitle = "Fear effect!",
    globalCameraMaxZoom = "Maximum camera distance",
    globalSection = "Global settings",
    guild = "Guild",
    hunterPackAspectFound = "Aspect of the Pack active",
    hunterPackAspectLine = "Aspect of the Pack active, remove it before combat!",
    hunterPackAspectSection = "Aspect of the Pack",
    hunterPackAspectTitle = "Aspect of the Pack!",
    kickFound = "The spell being cast can be interrupted",
    kickLine = "Spell interrupt possible!",
    kickSection = "Spell interrupt",
    kickTitle = "Spell interrupt",
    lockWindow = "Lock window",
    menuTitle = "GraalHelper",
    missNotifierSection = "Miss Notifications",
    noSpellsTracked = "No spells tracked yet.",
    notifiers = "Notifications",
    party = "Party",
    raid = "Raid",
    raidOrPartyOrNothing = "Raid/Party",
    reflectFound = "Spell Reflection active",
    reflectLine = "Do not cast - Reflect active!",
    reflectSection = "Reflect Window",
    reflectTitle = "REFLECT!",
    rootFound = "Root effect active",
    rootLine = "Root active!",
    rootSection = "Root effect",
    rootTitle = "Root effect!",
    say = "Say",
    sayCheck = "Show in chat",
    scale = "Scale",
    settings = "Settings",
    showBuffNames = "Show buff names",
    silenceFound = "Silence effect active",
    silenceLine = "Unable to cast spells - Silence active!",
    silenceSection = "Alert: Silence effect",
    silenceTitle = "Silence effect!",
    slashHide = "/gh hide - Hide alerts",
    slashLock = "/gh lock - Lock both",
    slashMenu = "/gh - Open menu",
    slashTest = "/gh test - Test both",
    slashUnlock = "/gh unlock - Unlock both",
    spellsHint = "Only tracked spells seen by the addon appear here.",
    spellsSection = "Tracked Spells",
    spellstealFound = "Stealable buffs found",
    spellstealLine = "Buffs ready to steal!",
    stealSection = "SPELLSTEAL",
    stealTitle = "SPELLSTEAL!",
    stunFound = "Stun effect active",
    stunLine = "Stun active!",
    stunSection = "Stun effect",
    stunTitle = "Stun effect!",
    summonNotifierSection = "Summon Notification",
    test = "Test",
    trackDisarm = "Disarm",
    trackFear = "Fear",
    trackKick = "Kick",
    trackReflect = "Reflect",
    trackRoot = "Root",
    trackSilence = "Silence",
    trackSteal = "Steal",
    trackStun = "Stun",
    trackUnknow = "Unknow",
    welcomeActive = "Enable welcome message",
    welcomeLock = "Lock welcome window",
    welcomeNotShow = "Do not show again",
    welcomeScale = "Welcome message scale",
    welcomeSentences = {
        "Where is the chicken?",
        "That's not wrong.",
        "Fat is life.",
        "We've had enough!"
    },
    welcomeTestButton = "Test welcome",
    welcomeTitle = "Welcome",
    yell = "Yell",

    MATCH = {
        SPELL_REFLECTION_NAMES = {
            ["Dark Shell"] = true,
            ["Spell Reflection"] = true,
            ["Reflective Magic Shield"] = true,
        },
        SPELL_SILENCE_NAMES = {
            ["Silence"] = true,
            ["Spell Lock"] = true,
            ["Arcane Torrent"] = true,
            ["Counterspell - Silenced"] = true,
            ["Kick - Silenced"] = true,
            ["Deafening Roar"] = true
        },
    },
}
